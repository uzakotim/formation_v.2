...
class OrbSlam : public nodelet::Nodelet {
...
void OrbSlam::onInit() {
  ...
  mrs_lib::ParamLoader param_loader(nh, "OrbSlam");
  ...
  transformer_ = std::make_unique<mrs_lib::Transformer>("OrbSlam");
  ...
  // sub_image_       = it.subscribe("image_in", 1, &OrbSlam::callbackImage, this);
  // sub_depth_       = it.subscribe("depth_in", 1, &OrbSlam::callbackImage, this);
  sub_camera_info_ = nh.subscribe("camera_info_in", 1, &OrbSlam::callbackCameraInfo, this, ros::TransportHints().tcpNoDelay());
  ...
  sync_->registerCallback(boost::bind(&OrbSlam::GrabRGBD, this, _1, _2));
  ...
  // | -------------------- initialize timers ------------------- |
  timer_check_subscribers_ = nh.createTimer(ros::Rate(_rate_timer_check_subscribers_), &OrbSlam::callbackTimerCheckSubscribers, this);

  ROS_INFO_ONCE("[OrbSlam]: initialized");
...
void OrbSlam::callbackCameraInfo(const sensor_msgs::CameraInfoConstPtr& msg) {
...
void OrbSlam::GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB,const sensor_msgs::ImageConstPtr& msgD) {
  ...
  ROS_INFO_THROTTLE(1, "[OrbSlam]: Total of %u images received so far", (unsigned int)image_counter_);

  // | ---------- Detect edges in the image using Canny --------- |

  /* find edges in the image */

  // const auto detected_edges = OrbSlam::detectEdgesCanny(bridge_image_ptr->image, low_threshold_);

  /* show the edges image in gui */

  // if (_gui_) {
    // OrbSlam::showEdgeImage(bridge_image_ptr->image, detected_edges);
  // }

  /* publish the image with the detected edges */

  // OrbSlam::publishOpenCVImage(detected_edges, msg_header, grayscale_encoding, pub_edges_);

  // | ----------- Project a world point to the image ----------- |

  /* find edges in the image */
  // const auto projection_image = OrbSlam::projectWorldPointToImage(bridge_image_ptr->image, msg_header.stamp, 0, 0, 0);
  ...
  /* publish the image with the detected edges */
  // OrbSlam::publishOpenCVImage(projection_image, msg_header, color_encoding, pub_projection_);

  /* publish image count */
  // OrbSlam::publishImageNumber(image_counter_);
...
void OrbSlam::callbackTimerCheckSubscribers([[maybe_unused]] const ros::TimerEvent& te) {
...
void OrbSlam::publishImageNumber(uint64_t count) {
...
void OrbSlam::publishOpenCVImage(cv::InputArray image, const std_msgs::Header& header, const std::string& encoding, const image_transport::Publisher& pub) {
...
cv::Mat OrbSlam::detectEdgesCanny(cv::InputArray image, int low_threshold) {
...
void OrbSlam::showEdgeImage(cv::InputArray image, cv::InputArray detected_edges) {
...
cv::Mat OrbSlam::projectWorldPointToImage(cv::InputArray image, const ros::Time& image_stamp, const double x, const double y, const double z) {
  ...
  if (!got_camera_info_) {
    ROS_WARN_THROTTLE(1.0, "[OrbSlam]: No camera info received yet, cannot backproject point to image");
  ...
  if (ret) {
    ...
    ROS_WARN_THROTTLE(1.0, "[OrbSlam]: Failed to tranform point from world to camera frame, cannot backproject point to image");
    return projected_point;
  }
  ...
  if (ret2) {
    ...
    ROS_WARN_THROTTLE(1.0, "[OrbSlam]: Failed to tranform point from world to drone frame, cannot project point");
    return projected_point;
  }
...
PLUGINLIB_EXPORT_CLASS(orb_slam::OrbSlam, nodelet::Nodelet);
