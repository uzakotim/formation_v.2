...
void BlobDet::GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB,const sensor_msgs::ImageConstPtr& msgD) {
  ...
  cv::Mat image_threshold     = BlobDet::ReturnBlueMask(image_HSV);
  // 4) finding contours
...
/*| --------- BlobDet Function --------------------------------|*/
  ...
  cv::Mat BlobDet::ReturnBlueMask(cv::Mat image)
  {
      ...
      cv::inRange     (image, BlobDet::color_min, BlobDet::color_max, image_threshold);
  ...
  cv::Mat BlobDet::ReturnRedMask(cv::Mat image)
  {
      ...
      cv::inRange     (image, BlobDet::color_one_min, BlobDet::color_one_max, mask1);
      cv::inRange     (image, BlobDet::color_two_min, BlobDet::color_two_max, mask2);
      total = mask1 | mask2;
